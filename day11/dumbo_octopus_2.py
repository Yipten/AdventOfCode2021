...
def find_simultaneous_flash(energy_levels):
    '''
    ...
    while True:
        ...
        for row in energy_levels:
            for j in range(len(row)):
                if row[j] > 9:
                    row[j] = 0
        # break out of loop if all octopuses flashed
        if num_flashes == 100:
            break
    return num_cycles
...
def main():
    ...
    print('First cycle with simultaneous flash: ' +
          str(find_simultaneous_flash(energy_levels)))
